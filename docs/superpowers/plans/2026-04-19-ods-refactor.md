...
**Goal:** Refactor `@ously/ods` to `@ously/ods` across the monorepo to align with the Oously Design System.
...
Modify: `packages/ods/tsconfig.json`
Change `"@ously/ods/*": ["./src/*"]` to `"@ously/ods/*": ["./src/*"]`.
...
Change `"@ously/ods": "workspace:*"` to `"@ously/ods": "workspace:*"`.
...
Change `transpilePackages: ["@ously/ods"]` to `transpilePackages: ["@ously/ods"]`.
...
Change `import { Button } from "@ously/ods";` to `import { Button } from "@ously/ods";`.
...
Replace all occurrences of `@ously/ods` with `@ously/ods`.

- [ ] **Step 2: Update historical docs**
Search for `@ously/ods` in `docs/` and replace with `@ously/ods`.

- [ ] **Step 3: Commit**
```bash
git add GEMINI.md docs/
git commit -m "docs: rename @ously/ods to @ously/ods"
```
